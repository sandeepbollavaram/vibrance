...
PRESETS: list[tuple[str, dict]] = [
    ...
    ("Cinema",     {"contrast": 18, "highlights": -18, "shadows": 14, "saturation": -8, "temperature": -10}),
...
class FilterGallery(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        for i, (name, _) in enumerate(PRESETS):
            ...
            btn.clicked.connect(
                lambda _c, n=name: self._on_click(n)
            )
    ...
    def refresh_thumbnails(self, source_bgr) -> None:
        ...
        scale = 120 / max(h, w)
        if scale < 1.0:
            thumb_src = cv2.resize(source_bgr, (int(w * scale), int(h * scale)), interpolation=cv2.INTER_AREA)
        else:
            thumb_src = source_bgr
        for btn, (name, overrides) in zip(self._buttons, PRESETS):
